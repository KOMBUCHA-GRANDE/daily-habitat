...
public class AuthController {
    ...
    public JwtTokenResponse refresh(@RequestBody @Valid RefreshRequest request) {
        return authService.refresh(request.refreshToken());
    }

    // 현재 기기 로그아웃 Authorization -> 엑시스 토큰 안에 uid 사용하여 리프레시 토큰 삭제 + 엑시스 블렉리스트
    @PostMapping("/logout")
    ...
    public ResponseEntity<Void> logoutAll(@RequestHeader(HttpHeaders.AUTHORIZATION) String authorization) {
